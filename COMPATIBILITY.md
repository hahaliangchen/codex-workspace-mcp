...
*   **转译方案**：在 [src/format_translate.rs](file:///d:/enterpriseProject/codex-workspace-mcp/src/format_translate.rs) 的 `emit_completion_events()` 中，将工具调用的最终输出格式从 Chat Completions 嵌套结构改为 Responses API 标准格式，每个工具调用作为独立顶层 `function_call` 条目推入 `final_output_items`，`id` 字段与流式 `response.output_item.done` 事件保持一致。

---

### 13. Shell Hook 伪装命令名称与参数字段不一致导致拦截失效（工具调用失败并降级）
*   **问题现象**：大模型尝试调用工作空间原生特权工具（如 `list_dir`、`read_file`）时，Codex 客户端直接将伪装指令执行并返回普通 Echo 字符串，代理层未能成功拦截并分发给本地 MCP 执行，导致大模型无法获得真实的文件目录数据，最终降级（Fallback）频繁直接运行系统原生的 `dir` 和 `Get-Content` 命令行。
*   **物理本质**：在流式转译发射（`ResponsesStreamConverter`）与最终拦截（`intercept_and_execute`）两端存在关键的**命名与参数不匹配（Mismatch）**：
    *   **命名冲突**：流转译层在 `process_tool_delta` 和 `emit_completion_events` 中将工具名伪装为 `"run_terminal_cmd"`，而拦截端（`agent.rs`）仅校验并拦截名为 `"exec_command"` 的工具；
    *   **参数冲突**：流转译层在构造伪装参数时使用 `"command"` 字段放置 Echo 与 Hex 载荷，而拦截端仅从 `"cmd"` 字段提取 `# PROXY_PAYLOAD: ` 标签。
    由于这两个不对齐，代理层判定其为普通 Shell 调用而直接放行，导致本地 MCP 工具从未被触发，模型最终因为获取不到数据而走普通终端降级。
*   **转译方案**：
    1.  在 [src/format_translate.rs](file:///d:/enterpriseProject/codex-workspace-mcp/src/format_translate.rs) 中，将 `ResponsesStreamConverter` 流式处理和完成状态中所有针对 `codex_workspace_mcp__` 前缀工具的包装名称及字段统一修改为 Codex 的白名单原生工具 `"exec_command"` 与 `"cmd"`；
    2.  在 [src/agent.rs](file:///d:/enterpriseProject/codex-workspace-mcp/src/agent.rs) 的拦截端 `intercept_and_execute` 与历史还原 `restore_history` 中，扩展兼容逻辑，使之能够同时匹配并兼容这两种主流命令行工具（`exec_command` / `run_terminal_cmd`）以及参数字段（`cmd` / `command`），完成双向闭环对齐。
